...
export default function PrintTab() {
  ...
  // บันทึก button of the blank designer rides the page's MAIN tab row (the
  // Products TabStrip) via the outlet context — same placement as Settings'
  // onActions slot. The designer's unmount cleanup clears it to null when
  // leaving blank mode / this route.
  const { setTabActions } = useOutletContext<ProductsOutletContext>()
    ...
    <div className="flex flex-col h-full min-h-0 gap-3">
      ...
      {mode === 'blank' ? (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin">
          <LabelDesigner profile="blank" onActions={setTabActions} />
        </div>
      ) : (
            ...
            right={
              mode === 'pricetag' ? (
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="primary-soft" size="lg" className="h-8 w-8 p-0" disabled={priceItems.length >= PRICE_MAX} onClick={openAdd} tooltip="เพิ่มสินค้า">
                    <Plus className="size-4" />
                  </Button>
                  <Button variant="elevated" size="lg" className="h-8 w-8 p-0" disabled={priceItems.length >= PRICE_MAX} onClick={() => setImportOpen(true)} tooltip="นำเข้า CSV — วางบาร์โค้ดจาก Excel หรือเลือกไฟล์ CSV">
                    <FileUp className="size-4" />
                  </Button>
                  <Button variant="outline" size="lg" className="h-8 w-8 p-0" disabled={priceItems.length === 0} onClick={clearAll} tooltip="ล้างทั้งหมด">
