...
export function ExpenseCategoriesTab() {
  ...
  const [snapshot, setSnapshot] = useState<ExpenseCategory[]>([])
  ...
  const openAdd = () => { setForm({ name: '', is_active: 1, sort_order: rows.length }); setDialog(true) }
  const openEdit = (c: ExpenseCategory) => {
    setForm({ id: c.id, name: c.name, is_active: c.is_active ?? 1, sort_order: c.sort_order })
    setDialog(true)
  }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await window.api.expenses.saveCategory(form)
  ...
  const saveReorder = async () => {
    try {
      ...
      await window.api.expenses.reorderCategories(rows.map(r => r.id))
      setReorderMode(false)
      await load()
