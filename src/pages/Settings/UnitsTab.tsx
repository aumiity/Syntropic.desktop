      ...
      <div className="bg-card rounded-card shadow-card overflow-hidden">
        ...
        <div className="[&>[data-slot=table-container]]:overflow-auto [&>[data-slot=table-container]]:scrollbar-thin border-l-8 border-r-8 border-card">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="min-w-[200px]">ชื่อหน่วย</TableHead>
                <TableHead className="text-right min-w-32">ตัวคูณ</TableHead>
            ...
            <TableBody>
              {rows.length === 0 ? (
                ...
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-right text-sm tabular-nums">{u.multiply}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex justify-center">
                      <Button className="w-16" size="icon-lg" variant="warm" onClick={() => openEdit(u)} title="แก้ไข">
                        <Edit />
                      </Button>
                    </div>
                  </TableCell>
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent size="sm">
          ...
          <DialogBody className="space-y-3">
            ...
            <FormField label="ตัวคูณ (กรณีเป็นหน่วยใหญ่)">
              <Input type="number" value={form.multiply ?? 1} onChange={e => setF('multiply', Number(e.target.value))} className="w-32" min={1} />
            </FormField>
          </DialogBody>
          <DialogFooter>
