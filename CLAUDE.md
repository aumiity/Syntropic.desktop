...
- **ELEVATED is the primary surface treatment for controls.** Every form/dialog/filter-strip Input, Textarea, Select, and neutral/cancel Button uses `variant="elevated"` (`bg-card` + `border` + `shadow-sm`) — NOT the bare `default`/`bg-input` filled look. The reference is `EditProduct/GeneralTab.tsx` (every field is `elevated`), not the (stale) showcase Modal demo. Exception: Button `default` stays = the primary teal CTA (save/confirm) — "elevated as primary" is about surfaces/inputs, not action buttons. → `docs/claude/ui-theming.md`
