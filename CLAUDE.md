...
### UI & theming
- **No emojis in program output.** Specifically: source code, UI strings/labels, and any visible runtime text. Use icon components for iconography and Badge variants + semantic color tokens for status indicators. OK to use emojis in plans, discussion, chat, and non-code comments (e.g. PR descriptions, planning docs) — the rule is about what ships to users, not how we talk about the work.
- **Icon libraries: `lucide-react` (outline, default everywhere) + `@tabler/icons-react` (opt-in where a solid look is wanted — e.g. `Sidebar.tsx` imports the `Icon*Filled` components directly).** Coverage isn't 100% for every concept (no plural "users" or "braces" filled icon) — pick the closest available `*Filled` icon rather than falling back to outline. Don't add a third icon package without updating this line. → `docs/claude/ui-theming.md`
...
- **Field variants = ONE shared set, `src/components/ui/field-variants.ts` is the SSOT.** All five field primitives (`Input`/`SearchInput`/`Textarea`/`SelectTrigger`/`NativeSelect`) import `fieldVariant()`. Every variant sits on the same elevated surface (`bg-card` + `border`) — the `variant` picks **shape × shadow** only: `default` (= `elevated` alias) = square+`shadow-sm` · `flat` = square, no shadow · `pill` = `rounded-full`+`shadow-sm` · `pill-flat` = `rounded-full`, no shadow. **Corner radius is a SEPARATE axis** — the `radius` prop (`sm`/`md`/`lg`, default `md`; square variants only, pill ignores it), a shared knob defined in `src/components/ui/radius.ts` and also on `Button`/`Badge`. (The square default `rounded-md` is deliberately NOT `--radius-control` — sanctioned carve-out, don't "fix" it.) A bare `<Input>` is already correct — do NOT hand-add `variant="elevated"`. **`filled` (flat `bg-input`) is DELETED 2026-07-17 — do not re-add.** Never hardcode radius/shadow on a field via `className` (`rounded-*`/`shadow-*`) — pick the `variant`/`radius` prop; a literal fights `fieldVariant()` and wins silently. **Mixing radii per-instance is the intended workflow** (operator decision 2026-07-17): explore with `radius="sm"`, `grep 'radius='` to find repeats, then promote a pattern to a default/convention. **Button is unaffected** (different namespace) — its `default` is still the primary teal CTA, never elevated. Reference: `EditProduct/GeneralTab.tsx`. **Dialog footer buttons by role:** a *lone* button = primary → `default` (neutral/positive: ปิด/ตกลง/บันทึก) or `destructive` (negative); *two* buttons = primary + secondary `elevated` (ยกเลิก/กลับ). `elevated` is NEVER used as the only button — it's always the secondary beside a `default`/`destructive` primary (`destructive-soft` = the soft-red tinted secondary slot beside a `destructive` primary, when a tinted secondary is wanted instead of neutral `elevated`). → `docs/claude/ui-theming.md`
...
- **Calendar dates display as `DD/MM/YYYY` — ONE format, no exceptions.** Always render a date through `formatDate()` (`src/lib/utils`, default `DD/MM/YYYY`) — or `formatDateTime()` for date+time (`DD/MM/YYYY HH:mm`). NEVER render a raw ISO string (`{row.expiry_date}` → shows `2026-06-13`), `toLocaleDateString`, or a `formatDate(x, '…')` override with a different display pattern. Christian era (ค.ศ.), not Buddhist. (Carve-out: date *pickers*/chart axes may use Thai month-name labels for navigation; printed receipt/tax-invoice documents may use formal Thai dates — both are deliberate, not date-field displays.) → `docs/claude/ui-theming.md`
- **Use the full color palette — don't default to primary/secondary/destructive everywhere.** Variants exist by role: `accent` (yellow), `primary-soft`, `info-soft`, `accent-soft` (cream/amber, formerly `warm`), `success`, `outline`, `ghost`, `destructive-soft`, plus the rich `*-outline` family. (No `tertiary`/`brand-soft`/`destructive2` — `tertiary`→`accent`, `brand-soft`→`primary-soft`, `destructive2`→`destructive-soft`.) → `docs/claude/ui-theming.md`
- **Scrollbars are THIN (7px) everywhere — one size, no exceptions.** The global `::-webkit-scrollbar` is 7px (`src/index.css`) and the `scrollbar-thin` utility matches it. Never define a different scrollbar width, a custom `::-webkit-scrollbar` size, or a thicker/thinner variant anywhere. → `docs/claude/ui-theming.md`
- **Table-card has 4 background zones** — only the column-header band is `bg-muted`; bottom status bar gets `border-t` only. List tables use elastic `min-w-`; spreadsheet grids use `table-fixed`+`w-[%]`. Row action buttons = square `size="icon-lg" variant="elevated"` (kebab "ตัวเลือก" + Popover menu; single action = direct elevated button) — neutral actions never role-tinted. **Exception:** delete + disable(Ban) actions use `elevated-destructive-soft`, the paired enable(RotateCcw) uses `elevated-success-soft`. → `docs/claude/ui-table-card.md`
- **All sizing is font-relative (rem) — never hardcode px to compensate when the font changes.** root font-size lives in `src/index.css` (`html { font-size: … }`, currently 18px) and the operator changes it up/down to taste until the real build. Tailwind `h-9`/`h-12`/`text-sm`/`size-4`/`gap-*`/`p-*` are rem → they scale together, so every height rule below holds at *every* font size and must NOT be edited when the font changes. Don't "pin" a size back with `h-[36px]` etc. — that one control stops scaling and desyncs. px figures quoted in docs ("`h-9` = 36px") are illustrative at a 16px root, not the rule. px is only correct where it must NOT scale with font: print sheets (A4/slip/label), 1-2px borders, 7px scrollbar, chart heights, window chrome. To bound a modal use `max-h-[Xvh]` (viewport, not font); to lock a row count use `h-[Nrem]` (scales with rows). → `docs/claude/ui-theming.md` (§Font-relative sizing)
- **Field-control height = `h-8` (2026-07-24).** The FIELD primitives are `h-8` — Input/SearchInput, Select/SelectTrigger/NativeSelect, Combobox, DateInput, DateRangePicker — plus the StatusFilter icon button (`h-8 w-8`). `h-9` fields looked too big/clunky; a brief same-day revert to `h-9` was reversed. **Do NOT flip these fields back to `h-9`.** **Button is separate and UNCHANGED — its ladder is `default` `h-8` / `lg` `h-9` / `xl` `h-10`; do NOT collapse `lg`→`h-8`.** Switch `framed="input"` = `h-8`; standard `framed` pill = `h-12`. → `.claude/memory/control_height_h9_revert.md`, `docs/claude/ui-table-card.md`
- **Standard table-card bars = `h-14` top / `h-12` bottom (LOCKED 2026-07-24 — intentionally different heights, don't equalize).** Top bar (title + filter strip) `h-14 px-4`; bottom bar (status/total/pagination) `h-12 px-5`; every control inside either bar = `h-8` (primitive default). The old "EVERY bar = h-12 / control = h-9" single-rule and its dead `h-14`/`h-10` filter-strip predecessor are both history. → `.claude/memory/table_card_bar_heights_locked.md`, `docs/claude/ui-table-card.md`
...
- **Thai UI language throughout.** Primary fonts (HARD): **Inter** (Latin) + **IBM Plex Sans Thai** (Thai), Noto Sans Thai fallback — the SAME in light AND dark (`--font-latin`/`--font-thai` in both `:root` and `.dark`). Other bundled faces are picker options only, not the default. Root font-size = variable (`html { font-size }` in `src/index.css`, currently 18px; operator tunes it until build) — all sizes are rem so they scale, never hardcode px to compensate (see Font-relative sizing invariant + `docs/claude/ui-theming.md`).
...
- **Quotation (ใบเสนอราคา) — DELETED 2026-06-11.** Module fully removed; sales documents offloaded to FlowAccount (not started). In-app Invoice subsystem cancelled. Full context + removed-file list: `.claude/memory/project_sales_documents.md`.

## Before a production build — remove DEV-only code
Strip the DEV-only previews/seeds/role-switch before compiling a real build — exact files + tags in `.claude/memory/project_prebuild_cleanup.md`. Also DROP the DEAD columns: `.claude/memory/project_refine_schema_checklist.md`.
