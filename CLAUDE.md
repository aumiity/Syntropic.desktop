...
- **Use the full color palette — don't default to primary/secondary/destructive everywhere.** Variants exist by role: `accent` (yellow), `primary-soft`, `info-soft`, `accent-soft` (cream/amber, formerly `warm`), `success`, `outline`, `ghost`, `destructive2`, plus the rich `*-outline` family. (No `tertiary`/`brand-soft` — `tertiary`→`accent`, `brand-soft`→`primary-soft`.) → `docs/claude/ui-theming.md`
