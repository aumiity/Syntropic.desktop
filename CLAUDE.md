...
> - **Renamed:** `is_vat` → `has_vat`
> - **Dropped from desktop schema:** `dosage_form_id`, `no_discount` (formerly `is_not_discount`), `unit_id` (formerly `unit_name`) — UI no longer sets them; columns removed via migration
> - **Added `is_drug` (Hygeia-style):** explicit "this product is a drug under the law" flag, gates the "ข้อมูลยา" section in EditProduct. `category` is now purely for sorting/filtering and never controls drug-related UI. Migration backfills `is_drug=1` for products that already had a `drug_type_id`.
> - **Base unit lives only in `product_units`:** every product is required to have exactly one row with `is_base_unit=1`. The `unit_name` shown in product list / POS / purchase / reports resolves via `LEFT JOIN product_units pu_base ON pu_base.product_id = p.id AND pu_base.is_base_unit = 1 LEFT JOIN item_units u ON u.id = pu_base.unit_id`. `products:create` inserts that base row in the same transaction as the product itself; the schema migration backfills it for existing rows (using each product's old `unit_id`, falling back to `'ชิ้น'` for nulls).
