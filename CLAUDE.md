...
- **Standard table-card layout (Products list and EditProduct tabs):**
  ...
  - **Column widths use `min-w-` NEVER `w-`/`table-fixed` (HARD).** Every `<TableHead>` carries a `min-w-XX` (or `min-w-[NNNpx]`) and the `<Table>` has **no** `table-fixed`. This keeps columns elastic — they stretch to fill a wide screen and shrink on a narrow one, but never collapse below the minimum. `w-XX` + `table-fixed` pins columns to a hard pixel width and wastes space / clips on resize — do not use it. Canonical example: `Products/index.tsx` (`min-w-14`, `min-w-[280px]`, `min-w-28`, …). Applies to **every** table in the app, not just the table-card layout.
...
Drag-to-reorder for table-card lists, built on `framer-motion` `Reorder` (already a dependency — do not add a DnD lib). `<SortableTableBody values onReorder>` renders the `<tbody>`; `<SortableRow value onDragEnd className>` renders one `<tr>` whose **first cell is a grip handle** (`dragListener={false}` + `useDragControls` — only the handle starts a drag, so a stray row click never reorders). Caller owns the list state (`values`/`onReorder`) and is responsible for persisting in `onDragEnd`.

- **Gate reordering behind an explicit mode toggle (HARD).** Never leave a list permanently draggable. The table-card header strip gets a `จัดลำดับ` button (`variant="info-soft"`, disabled when `< 2` rows); while on, swap the normal `<TableBody>` for `<SortableTableBody>` and the row's action column for the grip column, and show a `เสร็จสิ้น` (`variant="success"`) button to exit. This prevents accidental drags — the pattern users explicitly asked for. Canonical: `Settings/CategoriesTab.tsx`.
- **Backend renumbers, never the client.** Persist via an IPC that takes the ordered id array and rewrites `sort_order = 1..n` in **one transaction** (`settings:reorderCategories`). The list query stays `ORDER BY sort_order, id` (no UNIQUE on `sort_order` — ties just break by `id`, which is why a manual integer field was a bad UX and got replaced by drag). `onDragEnd` reads the latest order via a ref (the handler closes over stale state otherwise).
