...
- **⏸️ SUSPENDED 2026-07-17 — control-height rule is PAUSED during the UI redesign.** The operator is retuning control heights (`h-9 → h-8`, primitive-by-primitive) and radius per taste; **do NOT enforce the h-9 rule and do NOT revert `h-8` back to `h-9`.** The rule below is kept for reference and will be re-fixed (possibly at `h-8`) once the redesign settles. Bars stay `h-12` unless the operator says otherwise. → see [[radius-axis-prop]] for the same "tune now, codify later" mode.
- **EVERY bar = `h-12`, EVERY control inside = `h-9` — ONE rule, no exceptions.** _(paused — see above.)_ Header/title bar, filter strip, status/total bar — all `h-12`; every button/Input/Select/Combobox/DateInput/Switch inside = `h-9`. Button = `size="lg"` (already `h-9`); icon-only = `size="lg" className="h-9 w-9 p-0"`. The old split (filter strip `h-14`/`h-10`) is DEAD. Primitives that still bake `h-10` (combobox, date-input, date-range-picker, switch frame, NativeSelect) must read `h-9`. → `docs/claude/ui-table-card.md`
