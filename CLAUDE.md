...
- **Scrollbars are ONE size (10px) everywhere — no exceptions.** The global `::-webkit-scrollbar` is 10px (`src/index.css`) and the `scrollbar-thin` utility matches it. Never define a different scrollbar width, a custom `::-webkit-scrollbar` size, or a thicker/thinner variant anywhere. → `docs/claude/ui-theming.md`
