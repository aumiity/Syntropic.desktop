...
## ✅ DONE 2026-06-15: **Checkbox UI pass — destructive variant, qty-button parity, label-row h-12 sweep** — tsc-clean, NOT click-tested
- **`Checkbox` gains `variant="destructive"`** (`src/components/ui/checkbox.tsx`) — red checked-fill so negative rows read consistently (red row + red tick, was teal-on-red). Applied to ปิดใช้งาน/ซ่อน in EditProduct + EditBundle `GeneralTab`, and QuickStockDialog. `CheckRow variant="destructive"` now passes the red variant through to its inner Checkbox automatically. Showcase demo added in `/theme`.
- **Selection highlight ring+border → `border-2`** — drug-label list (`EditProduct/LabelsTab`) + preset card (`CSS/index.tsx`); dropped the doubled `ring-2` glow, kept soft bg tint. (ChoiceCard primitive, palette swatch, wizard step-glow left as-is by choice.)
- **Bundle component qty: `<Input>` → POS-style button + shared `QtyDialog`** (`EditBundle/ComponentsTab`) — same modal as the POS cart; removed the old `qtyDrafts`/`updateQty` string-draft logic. `QtyDialog` summary strip refactored so **คงเหลือ** (needs `stockQty`) and **รวม** (needs `unitPrice`) each render independently; bundle qty dialog passes neither + `presets={[]}` → bare stepper.
- **Label-only checkbox rows standardized to `h-12`** across forms: PrintTab ×9, SalesTab ×3, ReceiptSettingsTab, ExpenseFormDialog, LabelFormDialog ×3, LabelSettingsTab. Removed redundant "พักการใช้งาน" `CheckRow` from CustomerFormDialog + People (×2) — disable handled by the external/table button now (CheckRow import dropped). POS payment VAT + print-receipt toggles merged into ONE framed group (shared border + divide-y, each row h-12). DrugTypesTab อ.ย. flags → checkbox-LEFT/label-right + h-12 (house pattern). CustomerFormDialog "การแจ้งเตือนลูกค้า" header → h-12.
- **Bundled WIP (same commit):** barcode-sticker / price-tag printing overhaul (`lib/tags/*`, `lib/label/barcode.ts`, `lib/print/fonts.ts`, `PrintTab/GridEditor`, `TagProductSearchDialog`, `index.css`) + JetBrainsMono `.woff2` removed (font cleanup). Detail/SSOT: [[project_tag_printing]].
- Convention captured: [[checkbox-row-conventions]].
...
### Why
App had no real auth: `auth:getCurrentUser` was hardcoded to a seed user, passwords were plaintext, every IPC handler trusted the renderer's role (escalation trivial). Operator wanted real login (เลือกชื่อ + password), per-role permissions, and offline password recovery — see `docs/plans/User_Login_System.md` (plan v3, audited 2 rounds).
### What shipped — Slice 1 (Phase 0+1+2): login works end-to-end
- **Password hashing** `electron/auth/hash.ts` — `scrypt` (N=16384,r=8,p=1,salt16,key32), stored `scrypt$N$r$p$salt$hash`; legacy plaintext seed → compared once → upgraded to hash on first successful login. `genRecoveryCode()` = Crockford base32 `XXXX-XXXX-XXXX`.
- **Lockout** `electron/auth/lockout.ts` — `failed_attempts`+`locked_until` columns (main-process, survives restart); 5 fails → 30s backoff (delay, NOT permanent lock — admin can't lock themselves out forever).
- **Auth IPC** `electron/ipc/auth.ts` — `listLoginUsers` (returns id,name,role ONLY — no email/hash), `login` (checkLocked→verify→legacy-upgrade→clearFailures→returns id,name,role). Deleted hardcoded `getCurrentUser`.
- **saveStaff hardened** `electron/ipc/people.ts` — allow-list `['name','email','role','is_disabled']` (closed the `Object.keys` SQL footgun) + hash password main-side + password conditional (omit = don't touch).
- **Setup Phase 0** — `SetupWizard.tsx` now 4 steps (added "ตั้งรหัสผ่านผู้ดูแล") → `completeSetup` hashes admin password + generates recovery code (stores hash, shows plaintext once with copy button).
- **Session** `src/stores/userStore.ts` — in-memory only, `persist` removed (`current` starts null every launch → always go to Login). `login/logout/lock`; removed `hydrate`.
- **`LoginScreen.tsx`** wired to real IPC (was preview-only mockup; `preview` prop now DEV helper only). **`LoginGate`** in `App.tsx` (after SetupGate, before HashRouter). **TitleBar** user Popover (ล็อกหน้าจอ/ออกจากระบบ), renders only when `current != null`.
- Removed role `pharmacist` (admin/staff only).
### What shipped — Phase 3: main-side session + role enforcement + manager-override
- **Main-side session** `electron/auth/session.ts` — `Map<webContents.id,{userId,role}>` (in-memory). `bindSession` on login, `clearSession` on logout. **`electron/main.ts` clears session on webContents `destroyed` + `did-start-navigation` ONLY when `isMainFrame && !isSameDocument`** — HashRouter route changes are same-document so the session survives navigation (critical: otherwise role drops mid-use). `userStore.lock()/logout()` call `auth:logout` so an admin locking the screen can't leave staff inheriting admin.
- **`requireAdmin(e, override?)`** gates every admin-only IPC. **Manager-override = inline credential** (no token to leak): non-admin caller passes `{userId,password}`, verified main-side (must be role admin) with the same login lockout. `src/components/ui/manager-override-dialog.tsx` + `src/hooks/useManagerOverride.tsx` (`run(action,…)` → admin runs direct, staff gets the dialog).
- **`usePermission()`** `src/hooks/usePermission.ts` = UX layer only (hide nav/tabs). Sidebar `/reports`+`/settings` adminOnly; Manage ค่าใช้จ่าย tab + People พนักงาน tab hidden from staff.
- **Permission policy (operator-locked 2026-06-05):**
  - staff CAN create/edit general product data (`products:create/update` ungated)
  - **admin-only + override-able** (button shows for staff → manager dialog): ยกเลิกบิล (`reports:voidSale`), แก้ราคา (`products:updatePrice`), ปรับสต็อก/lot (`adjustStock`/`adjustLot`/`adjustLotBatch`/`updateLot`/`expireLot`), ยกเลิก GR (`purchase:cancel`)
  - **admin-only hidden** (no override): finance reports (financeSummary/salesPurchaseTrend/accountsPayable/topProducts/topSuppliers/cashierLeaderboard/salesStats/productVelocity/hourlyTraffic), `expenses:*`, settings writes, staff CRUD, backup/restore
  - **DeadStock**: staff sees the list but cost is stripped main-side (`reports:inactiveProducts` returns `cost_value: null` for non-admin — no DevTools leak) + DeadStock.tsx hides the cost column/footer when `!isAdmin`
  - **`completeSetup` NOT gated** (runs before any session), settings reads/POS/GR/customer ungated
### What shipped — Phase 2.5: recovery-code layer (self-service only)
- `auth:resetAdminPassword({recoveryCode,newPassword})` — SEPARATE lockout counter (`recovery_failed_attempts`/`recovery_locked_until`), verifies recovery_code_hash → resets ONLY admin password + regenerates code (new plaintext shown once) → does NOT bind a session, does NOT touch shop data. LoginScreen "ลืมรหัสผ่าน" flow built.
### Key files
- New: `electron/auth/{hash,lockout,session}.ts`, `src/hooks/{usePermission.ts,useManagerOverride.tsx}`, `src/components/ui/manager-override-dialog.tsx`, `src/pages/Manage/DeadStock.tsx`
- Updated: `electron/ipc/{auth,people,settings,reports,products,purchase,expenses,backup}.ts`, `electron/{main,preload}.ts`, `electron/db/schema.ts`, `src/stores/userStore.ts`, `src/App.tsx`, `src/pages/Auth/LoginScreen.tsx`, `src/components/layout/{TitleBar,Sidebar}.tsx`, `src/pages/Setup/SetupWizard.tsx`, `src/pages/People/index.tsx`, `src/pages/Manage/index.tsx`, `src/pages/Reports/Dashboard.tsx`, `docs/claude/ipc-api.md`
### Deferred / blocked (NOT in this work)
- **Vendor reset (Ed25519 signed token)** — BLOCKED: License infra (`electron/license/*`, Ed25519 keypair, key management) not started. LoginScreen has a "ลืม recovery code? ติดต่อผู้ขาย" placeholder. Needs a key-management decision first.
- **License/Activation (layer A) — entire system** — not started; no vendor account yet. See `docs/plans/License_Activation_System.md`.
- **must_change_password** flow — column intentionally NOT added in this slice.
### Carry-forward / click-test queue (Electron live — Hunter could not run a display)
1. **Session isolation:** admin → lock screen → staff login → DevTools `window.api.reports.financeSummary({})` must be `FORBIDDEN` (no admin inherit).
2. **Session survives navigation:** switch pages 5–6× (HashRouter) → sensitive IPC still works.
3. **Override:** staff voids bill / edits price / adjusts stock / cancels GR → dialog → admin password passes, staff/wrong password rejected.
4. **DeadStock:** staff sees no cost column; DevTools `inactiveProducts({})` → every `cost_value: null`. Admin sees costs.
5. **Recovery:** ลืมรหัส → recovery code → new password → new code issued → login with new password; recovery counter separate from login counter.
6. **Fresh install:** delete DB → SetupWizard 4 steps → set admin password → recovery code shown → Login.
- Plan: `docs/plans/User_Login_System.md`. Memory: `.claude/memory/project_user_login_licensing.md`, `.claude/memory/ipc-role-enforcement.md`.
...
- **Approach = silent HTML print** (reuse the proven label path, NOT the dead ESC/POS-over-TCP `printer:printReceipt`). New generic IPCs in `electron/ipc/printer.ts`: **`printer:printHtml`** (auto-measures content height for continuous 80mm roll → `px*25.4/96`mm, clamp 40–2000, `copies` loop, fixed-height fallback) + **`printer:previewHtmlPdf`** (A4 via `pageFormat`, or slip width+auto-height) → opens PDF in OS viewer.
- **Shared font helper extracted** → `src/lib/print/fonts.ts` (`FONT_REGISTRY`/`FONTS`/`esc`/`buildPrintFontFaceCss` — base64 @font-face embed; `LabelSettingsTab` now imports from here, no dup).
- **Builders** `src/lib/receipt/`: `buildSlipHtml.ts` (modes `receipt`/`abbrevTax`/`return`/`void`; abbrev tax invoice adds title + seller tax id + the mandatory **"ราคารวมภาษีมูลค่าเพิ่มไว้แล้ว"** ม.86/6), `buildTaxInvoiceHtml.ts` (A4 ม.86/4: seller+branch / buyer name-addr-taxid / VAT broken out / ต้นฉบับ-สำเนา), `normalizeSale.ts` (filters `is_cancelled`), `print.ts` orchestrator (`printSlip`/`previewSlip`/`printTaxInvoice`/`previewTaxInvoice`).
- **`receipt_settings` singleton table** (printer/width/height(0=auto)/auto_print/copies/font/notes/abbrev_tax_invoice) — handler uses the **ensure-row-then-UPDATE** pattern (NOT label_settings' INSERT-DEFAULT-then-skip bug, per audit). **`tax_invoices` table** (sale_id UNIQUE, buyer snapshot, `original_printed`, doc_no=invoice_no) + `electron/ipc/tax.ts` (`tax:issueOrGet` / `tax:get`; blocks voided/return). Added `settings.shop_branch` (+ ShopTab field).
- **UI**: `ReceiptSettingsTab.tsx` (live iframe preview via the real builder + ดูตัวอย่าง PDF / ทดสอบพิมพ์). **POS**: payment-modal checkbox "พิมพ์ใบเสร็จ" (default = `auto_print`), snapshot `lastSaleForPrint` captured **before** clearCart, print isolated in try/catch (print fail never fails the committed sale), reprint button in success dialog. **Manage/Sales row popover + SaleDetailDialog**: พิมพ์ใบเสร็จ + ใบกำกับภาษี (full = `TaxInvoiceBuyerDialog` with name/addr/13-digit-taxid validation; blocked for voided/return).
- Audit findings folded in pre-impl: ม.86/4-6 compliance, singleton-save pattern, isolated POS print fail-path, history reprint filters cancelled lines, auto-height mechanics, copies wiring.

### B) ระบบใบเสนอราคา (เฟส 1)
- `quotations` + `quotation_items` (snapshot customer + VAT rate; **issue_date set on create, immutable on update, renderer read-only**; never touches stock/lots). `electron/ipc/quotation.ts`: save (QT-YYYYMMDD-NNNN via **MAX-suffix + retry-on-UNIQUE-collision**, not bare COUNT — per audit), list/get/setStatus (transition guard), **edit/delete gated to `draft`** (per audit). VAT shown **broken out** (pre-tax + 7% + total), follows `sales_settings.vat`.
- A4 `buildQuotationHtml.ts` (reuses fonts + A4 structure) + `printQuotation`/`previewQuotation`. Pages: `src/pages/Quotation/QuotationList.tsx` (table-card clone of Manage/Sales: filter/status popover/pagination, status-gated row actions, derived `expired`) + `EditQuotation.tsx` (builder, read-only when !=draft) + `QuotationProductSearchDialog.tsx` (reuses `pos:searchProducts`). Routes `/quotation`,`/quotation/new`,`/quotation/:id/edit` + **sidebar "ใบเสนอราคา" under การขาย**.

### C) แปลงใบเสนอราคา → การขาย (เฟส 2)
- **State machine** `accepted → converting → converted` (+ release back to accepted). **Atomic claim** `quotation:beginConversion` (`UPDATE ... WHERE status='accepted'`, changes=0 → blocked) prevents double-sell across windows/terminals (audit High). **`converted` written via ONE path** `quotation:markConverted(id, invoice_no)` only (always records `converted_invoice_no`; removed from `setStatus`) (audit Med). **Block whole conversion** if any line can't be rebuilt — missing product_id / deleted / disabled (audit Med, operator chose all-or-nothing).
- `pos:getProductsByIds` (refactored `enrichProduct` shared with searchProducts) rebuilds cart lines with correct `selectedUnit`/`qty_per_base` so FEFO deducts the right base qty. `cartStore` gains `sourceQuotation`; `src/lib/quotation/loadToCart.ts` + `useConvert.tsx` (blocked-lines + replace-cart dialogs). POS shows "กำลังขายจากใบเสนอราคา QT-…" banner + cancel; on sale success `markConverted` runs best-effort (never fails the sale). Convert buttons on accepted; resume/cancel on converting rows.
- E2E proof: accepted QT → convert → POS cart (ACCU-CHEK 3 กล่อง @1,190, quoted price+unit) → pay → **บันทึกบิลสำเร็จ RC-20260531-0001** → quote = converted + linked invoice.

### Plans / audits (this session)
- `docs/plans/done/receipt_print.md`, `docs/plans/quotation.md`, `docs/plans/quotation-convert.md`
- `docs/audits/quotation-plan-audit.md`, `docs/audits/quotation-convert-plan-audit.md`

### Carry-forward / NOT done
- **Live click-test by operator** for all 3 (Playwright-verified, but no human checklist run). POS receipt auto-print flow + A4 full-tax-invoice buyer dialog verified via PDF preview, not a physical printer.
- Convert→sale phase-2 was intentionally deferred then built same session.
