...
## Status: ✅ Runnable — **Bundle creation hardened: atomic create + min-2-components invariant** (Session 2026-05-20f). No more orphan empty-bundle rows in DB. `BundlesList` "+ เพิ่มชุดสินค้า" now navigates to a virtual `/products/bundles/new` route; `EditBundle` handles a "new" mode that keeps the product + components in memory and commits them in a single transaction via new `products:createBundle` IPC — blocks save if `items < 2`. Existing `saveBundleItems` (edit-mode) gained the same `<2` guard. **Phase 5 / ขย.9** + Product Bundle Phase 1 + 2 + 6 audit fixes still standing. Type-clean across the board. **Bundle min-2 + ขย.9 + Bundle base + Manage/Reports Phase 1–4 + h-10 sweep all NOT click-tested yet.**
## Last updated: 2026-05-20
## Run: `npm run electron:dev`
## ⚠️ Next session:
##   1. **Click-test Bundle atomic create + min-2 guard** end-to-end — `/products/bundles` → "+ เพิ่มชุดสินค้า" → URL = `/products/bundles/new`, page shows form + "ยังไม่บันทึก" warning badge in meta card. Try saving with 0/1 components → main button "สร้างชุดสินค้า" is disabled with tooltip; if you somehow trigger it (e.g. via DevTools), the toast shows "ชุดสินค้าต้องมีรายการอย่างน้อย 2 รายการ". Add 2 components → button enables → click → atomic create → URL `replace`s to `/products/bundles/:id/edit` with success toast. **Back button on /new** → return to BundlesList; verify NO orphan row in DB. Edit-mode side: open an existing bundle, remove components until 1 left → ComponentsTab Save button disables + footer shows "ต้องมีรายการอย่างน้อย 2 รายการ (1/2)" in destructive color; backend `saveBundleItems` throws if bypassed.
