...
- **Font stack:** `var(--font-latin), var(--font-thai), sans-serif` — both swappable from the CSS font picker (`src/pages/CSS`).
  ...
  - **Latin picker options:** Inter, Google Sans, Sarabun, IBM Plex Sans Thai, IBM Plex Sans Thai Looped, Noto Sans Thai, Noto Sans Thai Looped, SF Thonburi, Bai Jamjuree, Anuphan — picker choices only, not the default. The four scoped Thai faces (IBM Plex / Noto, each ± Looped) expose an UNSCOPED `'… Latin'` twin (e.g. `'Noto Sans Thai Looped Latin'`) for the Latin slot — same split as `'Sarabun Latin'` — so the Latin option renders the real face, not a fallback.
  - **Bundled Thai alternates:** IBM Plex Sans Thai (Looped), Sarabun, SF Thonburi, Bai Jamjuree, Anuphan, Noto Sans Thai (+ Looped). Every picker font carries both scripts EXCEPT Inter, which has no Thai glyphs.
