...
12. **สัญญาของ `DateInput` (input วันที่) — invalid = `''`, เตือนด้วยกรอบแดง, required เช็คเองที่ parent (HARD).** `src/components/ui/date-input.tsx` ตรวจ "วันที่มีจริง" (เดือน 1-12, วันไม่เกินจำนวนวันของเดือน รองรับ leap year, ปี 1900-9999) ผ่าน `displayToIso()` แล้วบังคับสัญญา 3 ข้อ — **ห้ามถอยกลับ**:
    ...
    - **เพราะ invalid = `''` ช่องที่ required ต้องเช็ค `!value` เองที่ parent ตอน submit** (เช่น `if (!receiveDate) { toast(...); return }`). DateInput ตั้งใจไม่ block submit ให้ (ไม่มีจุดคอขวดกลาง — ปุ่มบันทึกอยู่ในแต่ละฟอร์ม). **ข้อความ toast ใช้แบบรวม `'รูปแบบวันที่ไม่ถูกต้อง'`** (ครอบทั้งว่าง+รูปแบบผิด เพราะ coerce-เป็นว่างทำให้ฟอร์มแยกสองกรณีไม่ออก) — กรอบแดงบนช่องเป็นตัวชี้ว่าช่องไหนผิด. ช่อง *optional* (วันหมดอายุที่สินค้าไม่มี exp ได้, `manufactured_date`, `dob`, `paidDate`/`orderDate` ตอนยังไม่ชำระ/ซื้อสด) จงใจไม่เช็ค — เว้นว่างได้. เส้นแบ่งคือ "ธุรกิจอนุญาตให้เว้นว่างไหม" ไม่ใช่ "เป็นช่องวันที่หรือเปล่า". ดู [[date-input-validation-contract]].
