...
8b. **Icon libraries — two allowed, don't add a third without updating this rule.** `lucide-react` = default outline icons, used everywhere unless a solid look is specifically wanted. `@tabler/icons-react` = opt-in for solid treatments — import the `Icon<Name>Filled` component directly (e.g. `IconPillFilled`) instead of the outline `Icon<Name>`. Coverage isn't 100% — some concepts have no filled twin (e.g. no `IconUsersFilled` for a plural-people glyph, no `IconBracesFilled`); when that happens pick the closest available filled icon for the concept (see `Sidebar.tsx` — `IconUserFilled` singular, `IconCodeCircleFilled` in place of braces) rather than silently falling back to the outline version. Both libraries' components accept `className` for Tailwind sizing (`size-N`, same as rule 8 above).
...
10. **Scrollbar — ขนาดเดียว 10px ทั้งโปรแกรม ห้ามมีขนาดอื่น (HARD).** global `::-webkit-scrollbar` ใน `src/index.css` ตั้งเป็น `w-[10px] h-[10px]` แล้ว ทั้งแอปจึงขนาดเดียวกันโดยอัตโนมัติ — ไม่ต้องคอยใส่คลาสเอง. utility `.scrollbar-thin` (10px เท่ากัน, ใช้อยู่ ~40 จุด — ชื่อ "thin" เป็นชื่อเดิม ที่ 10px ไม่ได้บางแล้วแต่คงชื่อไว้) ใช้ต่อได้เพื่อความชัดเจน. ประวัติ: 6px → 7px → **10px (2026-07-24 เจ้าของชอบ "กำลังดี")**. **ห้าม** กำหนดความกว้าง scrollbar อื่น หรือเขียน `::-webkit-scrollbar` ขนาดอื่นที่ใดก็ตาม.
...
- คลาส Tailwind spacing/size/typography (`h-8`, `h-12`, `h-14`, `text-sm`, `size-4`, `gap-2`, `p-4`, …) เป็นหน่วย **rem** → สเกลตาม root อัตโนมัติ. **กฏความสูงทั้งหมด (บาร์ `h-14` บน / `h-12` ล่าง, control ข้างใน `h-8` — ดู `ui-table-card.md`) ไม่ต้องแก้เมื่อ font เปลี่ยน** — บาร์กับ control ขยายพร้อมกัน สัดส่วนคงเดิมทุกขนาด.
...
- **px ที่ถูกต้องใช้ได้** = ค่าที่ตั้งใจให้ "ไม่สเกลตาม font" เท่านั้น: งานพิมพ์ A4/สลิป/label (px/pt เชิงกายภาพ), เส้นขอบ/divider 1-2px, scrollbar 10px, ความสูงกราฟ, มิติ window chrome (TitleBar). อย่าเอา px ไปใช้กับโครง layout ที่มีตัวอักษร rem อยู่ข้างใน.
...
As of 2026-07-17 all five field primitives share **one** variant set, exported from `field-variants.ts` as `FieldVariant` + `fieldVariant(variant, radius)`. Every variant sits on the same elevated surface (`bg-card` + `border border-border`). The `variant` picks **shape × shadow** only; **corner radius is a separate axis** (the `radius` prop):

| variant | shape | shadow |
|---|---|---|
| `default` (= `elevated` alias) | square (radius axis) | `shadow-sm` |
| `flat` | square (radius axis) | none |
| `pill` | `rounded-full` (radius ignored) | `shadow-sm` |
| `pill-flat` | `rounded-full` (radius ignored) | none |

- **Radius axis — the `radius` prop (`sm`/`md`/`lg`, default `md`).** Shared knob defined in `src/components/ui/radius.ts` (`type Radius` + `RADIUS_CLASS`), also carried by `Button` and `Badge`. On a field it sets the *square* corner (`sm` 0.25rem · `md` 0.375rem · `lg` 0.5rem); pill variants ignore it. **Mixing radii per-instance is the intended workflow** (operator decision 2026-07-17): explore with `radius="sm"`, `grep 'radius='` to find repeats, then promote a pattern to a primitive default or documented convention. The square default `rounded-md` is deliberately NOT `--radius-control` (0.5rem) — sanctioned carve-out, don't "fix" it to the token. (History: tried `rounded-sm` earlier the same day, then moved to a `radius` prop defaulting `md`.)
...
- **Never hardcode radius or shadow on a field via `className`.** No `rounded-*` / `shadow-*` at a call site — use the `radius` prop and the `variant`. `fieldVariant(variant, radius)` owns both; a literal in `className` fights it and wins silently.
- Adding a shape (e.g. a squared `sharp`)? Add it to `field-variants.ts` (`FIELD_SHADOW` / the pill set) — all five primitives inherit it for free — then add a `DemoRow` per primitive in `/theme`. A new radius size goes in `RADIUS_CLASS` in `radius.ts` (shared with Button/Badge).

Secondary action **Buttons** (the one paired *next to* a primary action, e.g. "ยกเลิก/กลับ" beside "บันทึก") still use `variant="elevated"` explicitly — Button was NOT part of the flip. See [[dialog-button-convention]]. **`elevated` is never used as the only button** — a lone footer button takes the primary role (`default` for neutral/close/OK, `destructive` for negative). Filter-strip controls are the `h-8` cluster (search + category select + filter/column popovers) — `h-8` is the **field-primitive default**, so do NOT pin it with `className="h-9"`; see [Field-control height](#field-control-height) below.

**The live reference is `src/pages/Products/EditProduct/GeneralTab.tsx`.**

### Field-control height

**Every field primitive is `h-8` — one number, everywhere, form and filter strip alike.** It is the primitive's own default, so a bare `<Input>` / `<SelectTrigger>` / `<Combobox>` / `<DateInput>` / `<DateRangePicker>` / `<NativeSelect>` already lands at `h-8`. `StatusFilterButton` matches it with `h-8 w-8`. **Textarea is the single exception** (multi-line → `min-h-16`).

- **Never pin a field's height at the call site.** `className="h-9"` on a field is always wrong — it makes that one control taller than its neighbours, and on `DateInput` it doesn't even work (the inner `<Input>` is hardcoded `h-8`; only the wrapper grows). Use `className` on a field for **width** only.
- **`h-9` belongs to `Button`, not to fields.** Button keeps its own ladder — `default` `h-8` / `lg` `h-9` / `xl` `h-10` — and must NOT be collapsed to match. That is why an icon button standing inside a table bar has to override back to `h-8 w-8 p-0`: it starts from the Button ladder, not the field default.
- History: the field primitives briefly went back to `h-9` on the morning of 2026-07-24 and were reversed the same day (`h-9` read "ใหญ่เกิน เทอะทะ"). **Do not flip them back.** → `.claude/memory/control_height_h9_revert.md`, `docs/claude/ui-table-card.md`
...
- **`DateInput` / `DateRangePicker`** — **`h-8`** (field default, same as Input/Select — see [Field-control height](#field-control-height)), calendar icon absolute on the right. **`className` targets the wrapper** (sizes/positions the whole component) — use it for width only (`className="w-44"`). **Never pass a height** (`h-9`, `h-10`, …): `DateInput`'s inner `<Input>` is hardcoded `h-8`, so a taller wrapper does NOT grow the field — it only leaves dead space and shifts the calendar button off-center.
- **Card radius** — every floating panel-card uses `rounded-card` (`--radius-card`, 1rem). Every control / control-panel (Button, SelectContent, PopoverContent) uses `rounded-lg` / `rounded-control` (`--radius`, 0.5rem). Don't mix `rounded-xl` / `rounded-2xl` literals; reach for the tokens. **Exception — the field primitives** (`Input`/`SearchInput`/`Textarea`/`SelectTrigger`/`NativeSelect`) plus `Button`/`Badge` carry a `radius` prop (`sm`/`md`/`lg`) from `radius.ts`; field square variants default `rounded-md` (0.375rem), not `--radius-control`. See the field-variants section above.
