...
Use `<Button size="icon-lg" variant="elevated">` with a single lucide icon and **no width override** — a square, NOT a wide `w-16` rectangle and NOT `size="sm" variant="ghost"`. Always give it a `title` for the tooltip/aria. `elevated` is the default variant for row-action triggers — do not role-tint the neutral ones (the old edit→outline / view→warm / open→primary-soft scheme is dead).
