...
- **Label-only checkbox row (label เดี่ยว ไม่มีคำอธิบาย) ในฟอร์ม/ตั้งค่า = `h-12`.** เป็นมาตรฐานใหม่ (เลิก h-9/h-10/h-11 เดิม). ใช้ `CheckRow className="...h-12 px-3"` หรือ `CheckRow framed className="h-12"`. หมายเหตุ: ตัวเลขนี้สูงกว่ากฎ control-in-bar = `h-8` โดยตั้งใจ — มันคือแถว setting ในกล่อง bordered ไม่ใช่ control ใน bar.
...
- **`CheckRow`/`Toggle` framed (label เดี่ยว) = `h-12` แล้ว** (เดิม bake `h-9` ที่ไม่เคย render จริง — call site override h-12 หมด; แก้ default ใน checkbox.tsx/switch.tsx 2026-06-16). **ยกเว้น `Toggle framed="input"`** (top-bar variant) = `h-8` ตามกฎ control-in-bar (แก้ 2026-08-13; เดิมโน้ตนี้เขียน `h-9` ผิด — `switch.tsx` เป็น `h-8` ตั้งแต่ [[control-height-h9-revert]]).
