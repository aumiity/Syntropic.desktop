...
3. **invalid = `''` ⇒ ช่อง required ต้องเช็ค `!value` เองที่ parent ตอน submit** (DateInput ไม่ block submit ให้ — ปุ่มบันทึกอยู่ในแต่ละฟอร์ม ไม่มีคอขวดกลาง). **ข้อความ toast ใช้แบบรวม `'รูปแบบวันที่ไม่ถูกต้อง'`** (ครอบทั้งว่าง+รูปแบบผิด เพราะฟอร์มแยกสองกรณีไม่ออกหลัง coerce; กรอบแดงชี้ช่องที่ผิดเอง). ผู้ใช้เลือกแบบรวม ไม่เอา per-field `onValidityChange` wiring.
