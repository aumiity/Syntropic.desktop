...
- **Quotation module DELETED 2026-06-11** (was HIDDEN 2026-06-02, now fully removed at operator's request — "คงไม่ได้ใช้แล้ว"). Removed: `src/pages/Quotation/*`, `electron/ipc/quotation.ts`, `src/lib/quotation/*`, `src/lib/receipt/buildQuotationHtml.ts`, `QuotationProductSearchDialog.tsx`, `quotations`/`quotation_items` schema tables + indexes + `converted_invoice_no` ALTER, `window.api.quotation` namespace, `Quotation*` types, POS convert-to-sale flow (cartStore `sourceQuotation` + banner + markConverted), routes, Sidebar link. Table definitions dropped from `schema.ts` (no `DROP TABLE` migration → existing DBs keep orphan tables harmlessly). tsc PASS (node + app). The print-template redesign work (`buildQuotationHtml.ts`, below) is moot — file deleted. **The build-from-scratch code is gone**; if quotation is ever revived it goes through FlowAccount, not a rebuilt in-app module.
