...
**พิมพ์ = OS dialog โดยตั้งใจ (อย่าแก้เป็น silent print):** ปุ่ม "พิมพ์" เปิด OS print dialog (เลือกเครื่อง + page range + จำนวนชุด ฟรีในตัว) — ไม่ใช้ silent `printer:printHtml`+`document_settings` แบบใบกำกับภาษี/สลิป/ฉลาก. เหตุผล (ผู้ใช้ตัดสิน 2026-06-17): รายงาน ข.ย. พิมพ์รายเดือนนาน ๆ ที + ต้องเลือกหน้าเองได้ (เผื่อกระดาษเสียแค่หน้าเดียว). silent print สงวนไว้สำหรับงานพิมพ์รัว ๆ ตอนขาย (สลิป/ฉลาก/ใบกำกับภาษี).

**⚠️ ห้ามใช้ `window.print()` ใน renderer:** Electron ไม่รองรับ Chromium print-preview → ขึ้น error "This app doesn't support print preview". ต้องสั่งพิมพ์ฝั่ง main ผ่าน IPC `printer:printVisible` → `event.sender.print({ silent:false, landscape:true, pageSize:'A4', margins:{marginType:'none'}, printBackground:true })` (เปิด system dialog จริง, honor @media print CSS ของหน้าปัจจุบัน). preload: `window.api.printer.printVisible({landscape})`; ทั้ง KhorYor9/KhorYorSaleLedger เรียกผ่าน `handlePrint` + toast ถ้า fail (ยกเว้น cancel).
