...
- วิธีที่ใช้จริง: helper `src/lib/print/printDomSheets.ts` → **serialize `.a4-sheet` ที่ render อยู่** (clone outerHTML เฉพาะหน้าที่เลือก) + inline app CSS ทั้งหมด (`document.styleSheets` cssRules, same-origin) + ฝังฟอนต์ไทย (`buildPrintFontFaceCss(getAppThaiFont())`) → ส่งเข้า **`printer:printDocument`** (IPC ใหม่ silent print เอกสารเต็มหน้า A4/A5) พิมพ์ไปเครื่องใน `document_settings` (''=default; PRINT_TO_PDF→เปิด PDF). **พิมพ์ออกมาเหมือนพรีวิวเป๊ะเพราะใช้ DOM/CSS ตัวเดียวกัน** (เลี่ยงปัญหา 2 render path / font metric เพี้ยน).
